...
def step(
    ...
    **kwargs
    ...
    if R.use_r():
        ...
        # - `python -m metaflow.main_cli flow <path_spec> …`
        flow_pathspec = None
        pcs = sys.argv[0].rsplit(os.sep, 2)
        if (
            len(pcs) == 3
            and pcs[-2:] == ["metaflow", "main_cli.py"]
            and sys.argv[1] == "flow"
        ):
            flow_pathspec = sys.argv[2]
        elif sys.argv[1:4] == ["-m", "metaflow.main_cli", "flow"]:
            flow_pathspec = sys.argv[4]

        if flow_pathspec:
            entrypoint = "%s -m metaflow.main_cli flow %s" % (executable, flow_pathspec)
        else:
