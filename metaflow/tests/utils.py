...
def run(flow, file=None, fn=None, cmd=None, args=None, entrypoint=None):
    ...
    try:
        ...
        n_runs_before = len(list(mf.Flow(name)))
    ...
    if cmd:
        ...
        f = flow(file=file, args=args, entrypoint=entrypoint, standalone_mode=False)
        assert f._success

    runs = list(mf.Flow(name))
...
def verify_output(cmd, expected_stdout, expected_stderr):
    ...
    actual_stdout = out.decode()
    actual_stderr = err.decode()
    assert actual_stdout == expected_stdout, ('"""%s""" != """%s"""' % (actual_stdout, expected_stdout))
    assert actual_stderr == expected_stderr, ('"""%s""" != """%s"""' % (actual_stderr, expected_stderr))
