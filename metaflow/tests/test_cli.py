...
# - `python -m metaflow.main_cli flow <file> show` (assumes file contains exactly 1 flow)
# - `python -m metaflow.main_cli flow <file>:<name> show`
...
def test_run(cmd, name):
    """Test `run`ing a flow via `python -m metaflow.main_cli flow <flow> run` and `metaflow flow <flow> run`"""
    flow = flows[name]
    file = flow["file"]
    expected = flow["data"]
    path_spec = "%s:%s" % (file, name)
